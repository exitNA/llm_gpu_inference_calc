    ...
    return [
        ...
        },
        {
            ...
            "steps": [
                ...
                build_calc_step("Prefill 理论参考卡数 G_pre", "G_pre = ceil(TPS_pre,target^peak / TPS_pre^card)", f"ceil({format_adaptive_tps(result['tps_pre_target_peak'])} / {format_adaptive_tps(result['prefill_tps_p95_card'])})", f"{result['prefill_gpu_count_by_throughput']} 卡", note="该值只表示按理论服务率线性扩展时的参考卡数，不直接并入主结果。"),
                build_calc_step("Decode 理论参考卡数 G_dec", "G_dec = ceil(TPS_dec,target^peak / TPS_dec^card)", f"ceil({format_adaptive_tps(result['tps_dec_target_peak'])} / {format_adaptive_tps(result['decode_tps_card'])})", f"{result['decode_gpu_count_by_throughput']} 卡", note="该值只表示按理论服务率线性扩展时的参考卡数，不直接并入主结果。"),
        ...
        {
            "title": "最终卡数与近似能力回推",
            "summary": "当前版本先按 G_mem 给出预估基线，再检查这组卡数是否满足 Prefill/Decode 理论参考卡数与时延必要条件。",
            "steps": [
                build_calc_step("业务基线卡数 G_req", "G_req = G_mem", f"{result['gpu_count_by_memory']}", f"{result['business_gpu_count']} 卡", note="当前版本缺少实测吞吐 profile，因此不再用 G_pre / G_dec 直接抬高主结果。"),
