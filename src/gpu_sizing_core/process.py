...
def build_calculation_process_sections(result: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "title": "吞吐必要条件与时延必要条件",
            "summary": "在缺少实测 profile 时，只把 Prefill/Decode 理论服务率用于必要卡数下界与单卡时延必要条件检查，不把它当作实测吞吐承诺。",
            "steps": [
                build_calc_step("Prefill 理论带宽上界", "TPS_pre,bw^card = B_mem × η_bw / b_pre(S_in,p95)", f"{format_calc_number(gpu.memory_bandwidth_gb_per_sec)} GB/s × {format_calc_number(runtime.bandwidth_efficiency, 2)} / {format_adaptive_memory(result['prefill_bytes_per_token_p95'])}", format_adaptive_tps(result["prefill_tps_p95_bw_limited"])),
                build_calc_step("Prefill 理论算力上界", "TPS_pre,cmp^card = F_peak × η_cmp / (2P_act + α_attn × S_in,p95)", f"{format_calc_number(get_peak_compute_tflops(gpu, runtime.precision))} TFLOPS × {format_calc_number(runtime.compute_efficiency, 2)} / (2P_act + {format_calc_number(result['prefill_flops_attention_coeff'])} × {traffic.p95_input_tokens})", format_adaptive_tps(result["prefill_tps_p95_compute_limited"])),
                build_calc_step("单卡 Prefill 理论服务率", "TPS_pre^card = min(TPS_pre,bw^card, TPS_pre,cmp^card)", f"min({format_adaptive_tps(result['prefill_tps_p95_bw_limited'])}, {format_adaptive_tps(result['prefill_tps_p95_compute_limited'])})", format_adaptive_tps(result["prefill_tps_p95_card"]), note="Prefill 使用 S_in,p95 作为保守长度口径；该值用于必要条件下界估算，不等同于实测吞吐。"),
                build_calc_step("Decode 理论带宽上界", "TPS_dec,bw^card = B_mem × η_bw / b_dec", f"{format_calc_number(gpu.memory_bandwidth_gb_per_sec)} GB/s × {format_calc_number(runtime.bandwidth_efficiency, 2)} / {format_adaptive_memory(result['decode_bytes_per_token'])}", format_adaptive_tps(result["decode_tps_bw_limited"])),
                build_calc_step("Decode 理论算力上界", "TPS_dec,cmp^card = F_peak × η_cmp / (2P_act)", f"{format_calc_number(get_peak_compute_tflops(gpu, runtime.precision))} TFLOPS × {format_calc_number(runtime.compute_efficiency, 2)} / {format_calc_number(result['decode_flops_per_token'])} FLOPs", format_adaptive_tps(result["decode_tps_compute_limited"])),
                build_calc_step("单卡 Decode 理论服务率", "TPS_dec^card = min(TPS_dec,bw^card, TPS_dec,cmp^card)", f"min({format_adaptive_tps(result['decode_tps_bw_limited'])}, {format_adaptive_tps(result['decode_tps_compute_limited'])})", format_adaptive_tps(result["decode_tps_card"]), note="Decode 同样使用理论服务率做必要条件下界估算，不等同于线上稳定输出速率。"),
                build_calc_step("Prefill 理论参考卡数 G_pre", "G_pre = ceil(TPS_pre,target^peak / TPS_pre^card)", f"ceil({format_adaptive_tps(result['tps_pre_target_peak'])} / {format_adaptive_tps(result['prefill_tps_p95_card'])})", f"{result['prefill_gpu_count_by_throughput']} 卡", note="该值只表示按理论服务率线性扩展时的参考卡数，不直接并入采购主结果。"),
                build_calc_step("Decode 理论参考卡数 G_dec", "G_dec = ceil(TPS_dec,target^peak / TPS_dec^card)", f"ceil({format_adaptive_tps(result['tps_dec_target_peak'])} / {format_adaptive_tps(result['decode_tps_card'])})", f"{result['decode_gpu_count_by_throughput']} 卡", note="该值只表示按理论服务率线性扩展时的参考卡数，不直接并入采购主结果。"),
        ...
        {
            "title": "最终卡数与近似能力回推",
            "summary": "当前版本先按 G_mem 给出采购基线，再检查这组卡数是否满足 Prefill/Decode 理论参考卡数与时延必要条件。",
            "steps": [
                build_calc_step("业务基线卡数 G_req", "G_req = G_mem", f"{result['gpu_count_by_memory']}", f"{result['business_gpu_count']} 卡", note="当前版本缺少实测吞吐 profile，因此不再用 G_pre / G_dec 直接抬高采购卡数。"),
                build_calc_step("Prefill 参考卡数是否满足", "G_req >= G_pre", f"{result['business_gpu_count']} >= {result['prefill_gpu_count_by_throughput']}", "满足" if result["prefill_necessity_met_at_baseline"] else "不满足"),
                build_calc_step("Decode 参考卡数是否满足", "G_req >= G_dec", f"{result['business_gpu_count']} >= {result['decode_gpu_count_by_throughput']}", "满足" if result["decode_necessity_met_at_baseline"] else "不满足"),
                build_calc_step("P95 总 Prefill 理论吞吐", "TPS_pre,p95^cap = G_req × TPS_pre^card(S_in,p95)", f"{result['business_gpu_count']} × {format_adaptive_tps(result['prefill_tps_p95_card'])}", format_adaptive_tps(result["cluster_prefill_tps_capacity_p95"])),
                build_calc_step("总 Decode 理论吞吐", "TPS_dec^cap = G_req × TPS_dec^card", f"{result['business_gpu_count']} × {format_adaptive_tps(result['decode_tps_card'])}", format_adaptive_tps(result["cluster_decode_tps_capacity"])),
                build_calc_step("理论保守可持续 QPS", "λ_p95^sus = min(TPS_pre,p95^cap / S_in,p95, TPS_dec^cap / S_out,p95)", f"min({format_adaptive_tps(result['cluster_prefill_tps_capacity_p95'])} / {traffic.p95_input_tokens}, {format_adaptive_tps(result['cluster_decode_tps_capacity'])} / {traffic.p95_output_tokens})", f"{format_calc_number(result['sustainable_qps_p95'])} req/s"),
                build_calc_step("理论保守日请求量", "Req_day^p95 = λ_p95^sus × 86400", f"{format_calc_number(result['sustainable_qps_p95'])} × 86400", f"{format_calc_number(result['daily_request_capacity_p95'])} req/day"),
