...
def build_calculation_process_sections(result: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "title": "业务目标折算",
            "summary": "先把业务输入折算成平均/峰值 token 工作量、在途预算和 decode 时间预算。",
            "steps": [
                build_calc_step("平均总长度", "S_avg = S_in,avg + S_out,avg", f"{traffic.avg_input_tokens} + {traffic.avg_output_tokens}", f"{traffic.avg_total_tokens} tokens"),
                ...
                build_calc_step("平均 Prefill 工作量", "TPS_pre,target^avg = λ_avg × S_in,avg", f"{format_calc_number(traffic.lambda_avg_qps)} × {traffic.avg_input_tokens}", f"{format_adaptive_tps(result['tps_pre_target_avg'])}"),
                build_calc_step("平均 Decode 工作量", "TPS_dec,target^avg = λ_avg × S_out,avg", f"{format_calc_number(traffic.lambda_avg_qps)} × {traffic.avg_output_tokens}", f"{format_adaptive_tps(result['tps_dec_target_avg'])}"),
                ...
                build_calc_step("平均在途预算", "C_avg^budget = λ_avg × E2E_avg", f"{format_calc_number(traffic.lambda_avg_qps)} × {format_calc_number(traffic.e2e_avg_target_sec)}", f"{format_calc_number(result['c_avg_budget'])} req"),
                ...
                build_calc_step("平均 Decode 时间预算", "T_dec,avg = E2E_avg - TTFT_avg", f"{format_calc_number(traffic.e2e_avg_target_sec)} - {format_calc_number(traffic.ttft_avg_target_sec)}", f"{format_calc_number(result['decode_avg_budget_sec'])} s"),
        ...
        },
        {
            ...
            "steps": [
                ...
                build_calc_step("平均单请求 Cache", "M_cache^req(S_avg) = L × S_avg × 每 token 每层 cache 字节数", f"{model.num_layers} × {traffic.avg_total_tokens} × ({_describe_cache_formula(model, runtime)})", format_adaptive_memory(result["avg_cache_bytes_per_request"])),
        ...
        {
            "title": "最终卡数与能力回推",
            "summary": "取 G_mem、G_pre、G_dec 最大值作为业务基线，再回推总吞吐、可持续 QPS 与显存并发余量。",
            "steps": [
                ...
                build_calc_step("常态可持续 QPS", "λ_avg^sus = min(TPS_pre,avg^cap / S_in,avg, TPS_dec^cap / S_out,avg)", f"min({format_adaptive_tps(result['cluster_prefill_tps_capacity_avg'])} / {traffic.avg_input_tokens}, {format_adaptive_tps(result['cluster_decode_tps_capacity'])} / {traffic.avg_output_tokens})", f"{format_calc_number(result['sustainable_qps_avg'])} req/s"),
