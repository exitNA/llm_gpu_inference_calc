...
*   `vram_gb` 保持厂商规格字段名，但当前实现按二进制容量解释，即 `1 GB(vram label) = 1024^3 bytes`。
*   `memory_bandwidth_gb_per_sec` 继续表示十进制 `GB/s` 规格值。
*   计算结果页里从字节换算出来的容量值，会使用二进制 `GiB / MiB / KiB` 展示，避免与吞吐规格值混淆。
