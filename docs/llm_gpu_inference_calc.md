...
其中带宽模型抓的是权重主项；attention 中间读写、softmax、cache 写入等二级访存开销统一吸收到 $\eta_{bw}$ 中。它仍然是采购前近似，而不是完整性能仿真。norm、rope、MLA 细节、路由与 kernel 实现差异仍然被吸收到 $P_{act}$、$\eta_{cmp}$ 和 $\eta_{inst}$ 中。
