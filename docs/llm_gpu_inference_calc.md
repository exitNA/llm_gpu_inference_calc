...
本文给出一套面向**采购前容量规划**的大模型在线推理 GPU 卡数测算方法。本文重点回答两个问题：

1. **在给定业务目标、模型参数和 GPU 参数的条件下，最少需要多少张 GPU 卡？**
2. **当 GPU 卡数确定后，这批卡大致能支撑到什么业务能力？**
...
后文将对总 Prefill / Decode 理论吞吐、理论保守可持续 QPS、理论保守日 token 总量、显存角度最大在途请求量，以及时延必要条件检查做近似说明。
...
全文按“**输入解析 -> 中间需求量 -> 三类约束 -> 主结果与能力回推**”的顺序展开。围绕三类约束：
...
当前实现存在明确的先后依赖关系，建议按下面的顺序理解：

1. 先解析业务输入，得到统一口径的 $\lambda_{peak}$、$C_{peak}^{budget}$、$S_{in,p95}$、$S_{out,p95}$、$S_{p95}$。
2. 再由 $\lambda_{peak}$ 与长度画像得到两类峰值工作量：
   - Prefill 峰值工作量 $TPS_{pre,target}^{peak}$；
   - Decode 峰值工作量 $TPS_{dec,target}^{peak}$。
3. 并行推进三条约束链路：
   - 显存链路：$M_w$、$M_r$、$M_{cache}$ -> $G_{mem}$；
   - Prefill 理论服务率链路：$TPS_{pre}^{card}$ -> $G_{pre}$；
   - Decode 理论服务率链路：$TPS_{dec}^{card}$ -> $G_{dec}$。
4. 最后汇总：
   - 采购主结果先取 $G_{req}=G_{mem}$；
   - $G_{pre}$ / $G_{dec}$ 仅作为理论参考卡数；
   - 时延目标只做单卡能力必要条件检查；
   - 基于 $G_{req}$ 做整体能力近似回推。

### 2.2 当前实现中的关键取值关系

从当前实现看，最容易影响结论的依赖关系有：
...
当前实现默认输出的就是上述 **P95 保守口径**，没有再单独给出平均长度画像下的能力回推。这里的线性叠加是**系统总服务能力近似**，适用于回答“总共能处理多少 token”，不意味着所有卡都能共同加速单个请求，也不应被理解为线上实测吞吐承诺。
...
当前实现默认输出的是 $\lambda_{p95}^{sus}(G)$，也就是 **P95 长度画像下的理论保守可持续 QPS**。若后续需要常态能力说明，可再额外引入平均长度画像做补充，但不应与当前默认输出混淆。
...
- 若 Prefill 必要条件不满足，则首字时延目标在当前单卡能力假设下偏紧；
- 若 Decode 必要条件不满足，则尾部生成时长目标在当前单卡能力假设下偏紧；
- 若系统实际运行点接近 $\lambda_{p95}^{sus}(G)$，即使单卡必要条件满足，排队和调度仍可能放大真实时延。

因此，时延相关结果更适合作为“必要条件是否满足”的解释项，而不是拍定一个精确时延值。
