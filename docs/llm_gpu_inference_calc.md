...
如需对照公开部署经验与 benchmark，可参考补充文档《[DeepSeek-R1 实际部署与测试结果摘录](./deepseek_r1_deployment_benchmarks.md)》。
...
- **显存约束**可以直接折算为总卡数下界；
- 在缺少实测 profile 时，本文中的**吞吐模型只作为必要条件下界估算**，用于回答“理论上至少不能低于什么卡数”，而不是给出实测吞吐承诺；
...
2. **吞吐必要条件**：峰值流量下，理论单卡服务率是否足以支撑输入 token 与输出 token 的总工作量下界；
3. **时延必要条件**：单卡服务能力是否至少能支撑目标 TTFT / E2E 预算。

其中：

- 显存约束给出总显存角度的卡数下界 $G_{mem}$；
- Prefill 吞吐必要条件给出输入工作量角度的理论参考卡数 $G_{pre}$；
- Decode 吞吐必要条件给出输出工作量角度的理论参考卡数 $G_{dec}$；
- 时延部分不再直接构造总卡数公式，而是提供**必要条件检查**。

在当前实现里，采购主结果先按显存约束给出：

$$
G_{req} = G_{mem}
$$

其中 $G_{pre}$ 与 $G_{dec}$ 仅保留为“若沿用这套理论服务率线性扩展，峰值工作量大致对应多少卡”的参考值，用于辅助判断当前显存基线是否与吞吐目标发生根本冲突。
...
在完成显存基线卡数测算后，还可以基于同一套总量模型对整体能力做近似回推，包括：

- 总 Prefill / Decode 理论吞吐；
- 可持续 QPS；
- 日 token 总量；
- 最大在途请求量；
- 时延目标是否存在明显风险。
...
- 当前版本最终卡数先取 `G_mem`；
- `G_pre` / `G_dec` 作为理论参考卡数保留，用于检查显存基线是否与目标工作量发生根本冲突；
- 时延目标不直接折算为总卡数，而是作为单卡能力层面的必要条件检查。

交互界面中的“计算图”会按这条依赖链，把输入口径、中间指标、约束判断和最终结果并排展示。调整任一输入后，相关节点会同步刷新，用于直观看到参数影响路径。
...
因此，当前代码中的吞吐必要条件始终以**解析后的峰值 QPS + P95 长度**为准；显存约束则以**解析后的峰值在途预算 + P95 总长度**为准。
...
这里回答的是：**在缺少实测数据时，峰值流量下系统每秒需要处理多少 token，而理论单卡服务率至少要达到什么量级，才能不与该工作量发生根本冲突。**

因此，本文中的 Prefill / Decode 结果应理解为**必要条件下界估算**：若连该下界都无法满足，则当前方案一定不够；若表面满足，也不代表线上真实吞吐一定能达到。

### 6.2 单卡 Prefill 理论服务率
...
由于 Prefill 常对应长输入，attention 计算不应忽略。因此，单卡 Prefill 理论服务率写为带宽受限与算力受限两种上界中的较小者：
...
因此，若已有线上或离线 profile 数据，应优先用实测单卡吞吐替代理论公式；在没有实测数据时，本文公式只应用作必要条件下界和量级关系说明。
...
- $G_{mem}$ 反映总显存量约束；
- $G_{pre}$ 反映峰值输入 token 工作量在理论服务率模型下对应的参考卡数；
- $G_{dec}$ 反映峰值输出 token 工作量在理论服务率模型下对应的参考卡数。

因此，当前实现中的采购主结果由显存基线给出；$G_{pre}$ 与 $G_{dec}$ 只用于辅助判断这组卡数是否与吞吐目标发生根本冲突。
...
- 主导约束项（当前版本固定为显存）；
- 显存约束对应卡数 $G_{mem}$；
- Prefill 吞吐必要条件对应参考卡数 $G_{pre}$，以及当前显存基线是否满足；
- Decode 吞吐必要条件对应参考卡数 $G_{dec}$，以及当前显存基线是否满足；
- Prefill 时延必要条件是否满足；
- Decode 时延必要条件是否满足；
- 关键输入口径与效率系数；
- 额外安全系数或风险说明。
...
这些回推结果仍是采购前的近似值，并且建立在前文“吞吐必要条件”模型的线性叠加之上。它们更适合作为预算评审、资源冗余分析和能力说明，而不是线上 SLA 承诺。

### 9.2 总 Prefill / Decode 理论吞吐回推

在总量模型下，可将 $G$ 张 GPU 的系统总理论吞吐近似写为单卡服务率的线性叠加：
...
- $TPS_{pre}^{cap}(G; S)$ 表示在输入长度 $S$ 下系统总 Prefill 理论吞吐；
- $TPS_{dec}^{cap}(G)$ 表示系统总 Decode 理论吞吐。
...
这里的线性叠加是**系统总服务能力近似**，适用于回答“总共能处理多少 token”，不意味着所有卡都能共同加速单个请求，也不应被理解为线上实测吞吐承诺。

### 9.3 可持续 QPS 回推

给定长度口径后，可用系统总 Prefill / Decode 理论吞吐分别折算出相应的可持续请求到达率上限。
...
已知系统总理论吞吐后，可进一步回推 24 小时内可持续处理的 token 总量。
...
- 已知总卡数 $G$ 下的总 Prefill 理论吞吐；
- 已知总卡数 $G$ 下的总 Decode 理论吞吐；
- 理论保守可持续 QPS $\lambda_{p95}^{sus}(G)$；
...
- 单卡 Prefill 实测服务率 $TPS_{pre}^{card}(S)$；
- 单卡 Decode 实测服务率 $TPS_{dec}^{card}$；
...
- 用显存约束估算总显存量对应的卡数下界；
- 用 Prefill / Decode 吞吐必要条件估算峰值工作量对应的理论参考卡数；
- 当前实现先取显存约束作为采购基线：
...
在此基础上，本文进一步给出了一套已知卡数后的整体能力近似回推方法，用于回答：

- 这批卡在必要条件模型下的总吞吐大致有多大；
- 在必要条件模型下能支撑的可持续 QPS 大致有多高；
- 在必要条件模型下一天内大致能处理多少 token 和多少请求；
- 显存角度最多能容纳多少在途请求；
- 时延目标是否存在明显根本性风险。
...
1. 先用显存公式得到采购基线卡数；
2. 再用吞吐必要条件参考卡数与时延必要条件，判断这组显存基线卡数在体验目标上是否存在根本性矛盾；
