...
本阶段回答的问题是：**业务侧给出 QPS 和时延目标后，系统在 Prefill 和 Decode 两个阶段分别至少要提供多少 token 吞吐？时延目标又如何直接进入后续实例数求解？**
...
因此，TTFT 主要约束 Prefill，E2E 与输出长度主要约束 Decode。本文在采购前测算中采用一个显式假设：

> **将 TTFT 近似视为模型 Prefill 时间。**

这意味着排队、调度、网络和服务栈开销不再单独拆分，而是统一吸收到效率系数和结果保守性中。这样做的原因是：采购前通常很难稳定获得这些非模型开销的分解数据；与其引入新的难以验证超参数，不如直接采用更清晰、可复核的近似口径。后文的需求推导、实例求解与一致性校验都基于这一假设保持一致。
...
其中 $T_{dec,avg}$ 与 $T_{dec,p95}$ 分别表示平均与 P95 场景下，从首字出现到请求结束可分配给 Decode 的时间窗口。
...
### 5.4 时延目标如何直接进入实例计算主流程

在 QPS 驱动口径下，单位时间 token 工作量由第 5.2 节给出；而 TTFT 与 E2E 并不会改变单位时间必须处理的 token 总量。它们进入主流程的方式有两条：

- **E2E 通过第 5.3 节的在途请求预算进入显存约束**，从而直接影响 $N_{inst}^{mem}$；
- **TTFT 与 $T_{dec}$ 通过阶段时延约束进入 Prefill / Decode 实例数求解**。在当前两阶段稳态近似下，这些时延约束可改写为阶段吞吐约束，因此会直接参与第 8 章的主公式，而不再只是后验检查。

换句话说，后文不会再把 TTFT / E2E 仅仅作为第 9 章的效果反推目标，而是把它们显式写进实例数下界的推导过程中。
...
若单实例在 P95 输入长度下的 Prefill 吞吐为 $TPS_{pre}^{inst}(S_{in,p95})$，则为了满足峰值 Prefill token 工作量，至少需要

$$
N_{inst}^{pre,tps}=\left\lceil\frac{TPS_{pre,target}^{peak}}{TPS_{pre}^{inst}(S_{in,p95})}\right\rceil
$$
...
在峰值场景下，若每个请求的 Prefill 时间目标为 $TTFT_{p95}^{target}$，则系统在任一时刻大致需要同时容纳的 Prefill 阶段请求数预算可写成

$$
C_{pre,p95}^{req}=\lambda_{peak}TTFT_{p95}^{target}
$$

而单实例在该目标下可支撑的 Prefill 阶段请求数近似为

$$
C_{pre}^{inst,lat}=\frac{TPS_{pre}^{inst}(S_{in,p95})}{S_{in,p95}/TTFT_{p95}^{target}}
$$

因此，满足 Prefill 时延目标至少需要

$$
N_{inst}^{pre,lat}=\left\lceil\frac{C_{pre,p95}^{req}}{C_{pre}^{inst,lat}}\right\rceil
$$

个实例。将上式展开可得

$$
N_{inst}^{pre,lat}=\left\lceil\frac{\lambda_{peak}S_{in,p95}}{TPS_{pre}^{inst}(S_{in,p95})}\right\rceil
$$

可见在当前两阶段稳态近似下，Prefill 时延约束与 Prefill 吞吐约束化简后给出相同的数值下界；但把它显式写出，有助于看清 **TTFT 已直接参与实例数主流程**，而不是仅在后文校验中出现。

#### 8.2.3 Decode 吞吐约束

若单实例 Decode 吞吐为 $TPS_{dec}^{inst}$，则为了满足峰值 Decode token 工作量，至少需要

$$
N_{inst}^{dec,tps}=\left\lceil\frac{TPS_{dec,target}^{peak}}{TPS_{dec}^{inst}}\right\rceil
$$
...
P95 场景下的 Decode 时间预算为 $T_{dec,p95}=E2E_{p95}^{target}-TTFT_{p95}^{target}$。于是，峰值场景下系统需要同时容纳的 Decode 阶段请求数预算近似为

$$
C_{dec,p95}^{req}=\lambda_{peak}T_{dec,p95}
$$

单实例在该目标下可支撑的 Decode 阶段请求数近似为

$$
C_{dec}^{inst,lat}=\frac{TPS_{dec}^{inst}}{S_{out,p95}/T_{dec,p95}}
$$

因此，满足 Decode 时延目标至少需要

$$
N_{inst}^{dec,lat}=\left\lceil\frac{C_{dec,p95}^{req}}{C_{dec}^{inst,lat}}\right\rceil
$$

个实例。将上式展开可得

$$
N_{inst}^{dec,lat}=\left\lceil\frac{\lambda_{peak}S_{out,p95}}{TPS_{dec}^{inst}}\right\rceil
$$

在当前两阶段稳态近似下，Decode 时延约束与 Decode 吞吐约束同样化简为相同的数值下界；但它明确体现了 $E2E-TTFT$ 这一阶段时间预算已经直接参与主流程。

#### 8.2.5 显存约束

在 QPS 驱动口径下，吞吐约束由单位时间 token 工作量决定，而显存约束则通过在途请求预算体现时延目标对 KV cache 压力的影响。若单实例在显存上最多承载 $N_{req}^{mem}$ 个保守长度请求，而按 QPS 与时延目标反推得到的峰值在途请求规模近似为 $C_{peak}^{budget}$，则为了容纳这些在途请求，至少需要
...
显存、Prefill 吞吐、Prefill 时延、Decode 吞吐、Decode 时延这几类约束必须同时满足，因此业务所需的最小实例数为

$$
N_{inst}^{biz}=\max\left(N_{inst}^{mem},\,N_{inst}^{pre,tps},\,N_{inst}^{pre,lat},\,N_{inst}^{dec,tps},\,N_{inst}^{dec,lat}\right)
$$

在当前两阶段稳态近似下，$N_{inst}^{pre,tps}=N_{inst}^{pre,lat}$，且 $N_{inst}^{dec,tps}=N_{inst}^{dec,lat}$。因此，最终实现时也可以退化成更紧凑的三项写法；但在方法论上，将时延约束显式展开更能说明 TTFT 与 E2E 已直接参与实例数主流程。
...
它的单位是输出 token/s。这里更准确地说，它表示峰值流量窗口内单请求平均可分到的 Decode 吞吐份额，而不是用户逐 token 观测到的严格流式速度。

### 9.4 时延目标一致性校验

在 QPS 驱动口径下，若不额外引入排队模型，仅凭总吞吐很难直接精确反推出真实 TTFT 与 E2E。因而本文不再把这一节写成“时延数值预测”，而是改为：**检查当前规模在目标时延口径下，是否具备足够的阶段承载能力**。

平均场景下，系统需要同时容纳的 Prefill 阶段请求数预算为

$$
C_{pre,avg}^{req}=\lambda_{avg}TTFT_{avg}^{target}
$$

而当前规模在平均输入长度口径下可支撑的 Prefill 阶段请求数近似为

$$
C_{pre,avg}^{cap}=\frac{TPS_{pre}^{cluster}}{S_{in,avg}/TTFT_{avg}^{target}}
$$

若满足

$$
C_{pre,avg}^{cap}\ge C_{pre,avg}^{req}
$$

则说明当前规模在平均口径下与 TTFT 目标一致。

同理，平均场景下 Decode 阶段请求数预算为

$$
C_{dec,avg}^{req}=\lambda_{avg}T_{dec,avg}
$$

当前规模在平均输出长度口径下可支撑的 Decode 阶段请求数近似为

$$
C_{dec,avg}^{cap}=\frac{TPS_{dec}^{cluster}}{S_{out,avg}/T_{dec,avg}}
$$

若满足

$$
C_{dec,avg}^{cap}\ge C_{dec,avg}^{req}
$$

则说明当前规模在平均口径下与 E2E 目标一致。

在保守的 P95 口径下，需检查

$$
C_{pre,p95}^{cap}=\frac{TPS_{pre}^{cluster}}{S_{in,p95}/TTFT_{p95}^{target}}\ge \lambda_{peak}TTFT_{p95}^{target}=C_{pre,p95}^{req}
$$

以及

$$
C_{dec,p95}^{cap}=\frac{TPS_{dec}^{cluster}}{S_{out,p95}/T_{dec,p95}}\ge \lambda_{peak}T_{dec,p95}=C_{dec,p95}^{req}
$$

若两式均成立，则说明当前规模在保守高分位口径下与目标时延一致。

在当前两阶段稳态近似下，这些一致性校验在代数上会化简回第 8 章中的阶段吞吐 / 时延实例数约束；但把它们写成阶段请求数预算与阶段承载能力的比较，更便于理解 TTFT 与 E2E 在主流程中的作用边界。

需要强调的是：以上检查仍然不是严格的 SLA 证明。当实际请求到达率逼近或超过第 9.2 节反推的可持续 QPS 能力时，排队与调度效应会迅速放大，真实时延可能显著劣化。

### 9.5 在途请求数反推

在当前口径下，若第 9.4 节中的阶段时延一致性校验成立，则平均在途请求数可继续按目标时延近似为

$$
C_{avg}^{est}\approx \lambda_{avg}E2E_{avg}^{target}
$$

在保守的峰值口径下，峰值在途请求数上界可继续按目标时延预算近似为

$$
C_{peak}^{upper}\approx \lambda_{peak}E2E_{p95}^{target}
$$

因此，第 9.5 节更准确地说是在复核：当前规模下所对应的在途请求规模，是否与第 5.3 节用于显存约束的在途请求预算同一量级。这一结果既可用于验证第 8 章显存约束里采用的在途请求预算是否合理，也可帮助判断当前方案的 KV cache 压力主要出现在平均流量还是峰值流量窗口。
